...
type decl_binding = name * value [@@deriving eq]
...
module Interpret (  M : MONAD_FAIL) : sig
  val run : progr -> (interpret_ok, interpret_err) M.t
end = struct
  open M
  ...
  let rec eval expr env =
    ...
    | EBinopr (op, e1, e2) -> (
        ...
        | _ -> (
            ...
            | VInt x, VInt y, Leeq -> return (vbool (x <= y))
            | VInt x, VInt y, Gre -> return (vbool (x > y))
            | VInt x, VInt y, Greq -> return (vbool (x >= y))
...
let run_interpret code = 
  let helper = parse prog code in
  match helper with
  |Ok parsed -> InterpretResult.run parsed
  |Error err -> 
